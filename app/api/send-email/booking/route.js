...
export async function POST(request) {
  ...
  try {
    const formDataSendEmail = await request.json();
    const { name, email, phone, date, time } = formDataSendEmail;
    ...
    await transporter.sendMail({
      ...
      <div style="font-family: Arial, sans-serif; color: #e2ded4; background-color: #002b5b; padding: 20px; border-radius: 8px; box-shadow: 0px 20px 30px rgba(0, 0, 0, 0.2); max-width: 600px; margin: auto;">
        <div style="text-align: center; margin-top: 20px;">
          <img src="https://www.aceyourscore.com/logo.png" alt="Profile Image" style="max-width: 100px; border-radius: 50%;"/>
        </div>   
